  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#0c0c0c] border border-white/10 rounded-xl p-0 w-full max-w-4xl h-[70vh] shadow-2xl relative flex flex-col overflow-hidden"
        onClick={e => e.stopPropagation()}
      >
        {/* Terminal Header */}
        <div className="flex items-center justify-between px-4 py-2 bg-black border-b border-white/5">
        ...
        <div className="flex-1 overflow-y-auto p-4 font-mono text-xs sm:text-sm text-green-400 bg-transparent custom-scrollfoo leading-relaxed whitespace-pre-wrap">
          {logs.length === 0 ? (
            <span className="text-white/20 italic">Aguardando logs...</span>
          ) : (
            logs.map((log, index) => (
              <div key={index} className="mb-1 opacity-90 hover:opacity-100 hover:bg-white/5 px-1 rounded">
                {log}
              </div>
            ))
          )}
